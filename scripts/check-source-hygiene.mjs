...
const roots = new Set([
  ...
  // Barrel index.ts — намеренные точки входа агентского контракта (TypeScript-as-truth).
  // Они не обязаны импортироваться рантаймом, но фиксируют public API подмодуля
  // для AI-агентов и `npm run typecheck`. См. @packageDocumentation в каждом файле.
